...
def checkSemantics(string):
	...
	for i in range(len(catTab)):
		if catTab[i] == 'undefined' and symTab[i] in string:
...
def generateAST():
	...
	for i in forexpr:
		print(astExpr(i)+", ",end="");
	print(astExpr(incr),")");
with open('inp2.c') as f:
	s = f.read();
s1 = removeComments(s)
s1 = s1.replace("\n\n","\n");
with open('removecomments.c','w+') as f1:
	f1.write(s1);


count = 0;
with open('removecomments.c') as f2:
	code = f2.read();
...
for i in icg_lines:
	if "for" in i:
		...
		intermediate += "ifFalse "+str(cond)+" goto L"+str(labelCount)+"\n";
	elif "=" in i:
		icgExpr(i);
